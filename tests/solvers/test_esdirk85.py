...
class ESDIRK85Test(unittest.TestCase):
    """
    Test the implementation of the 'ESDIRK85' solver class
    """

    def setUp(self):
        self.solver = ESDIRK85(initial_value=1, func=func, jac=jac, tolerance_lte=1e-6)
    ...
    def test_integrate_fixed(self):
        ...
        timesteps = np.logspace(-0.2, 0, 10)
        errors = []
        ...
        #test convergence order, expected ca. 7 or 8
        p, _ = np.polyfit(np.log10(timesteps), np.log10(errors), deg=1)
        self.assertEqual(round(p), 7)
