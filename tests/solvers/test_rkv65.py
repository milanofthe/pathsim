...
class RKV65Test(unittest.TestCase):
    """
    Test the implementation of the 'RKV65' solver class
    """

    def setUp(self):
        self.solver = RKV65(initial_value=1, func=func, jac=jac)
    ...
    def test_integrate_fixed(self):
        ...
        #test convergence order, expected >= 6
        p, _ = np.polyfit(np.log10(timesteps), np.log10(errors), deg=1)
        self.assertGreater(p, 6)
    ...
    def test_integrate_adaptive(self):
        ...
        self.solver.reset()
        time, numerical_solution = self.solver.integrate(time_start=0.0, time_end=1.0, dt=10, adaptive=True)
