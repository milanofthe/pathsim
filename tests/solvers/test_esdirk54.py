...
class ESDIRK54Test(unittest.TestCase):
    """
    Test the implementation of the 'ESDIRK54' solver class
    """

    def setUp(self):
        self.solver = ESDIRK54(initial_value=1, func=func, jac=jac, tolerance_lte=1e-6)
    ...
    def test_integrate_fixed(self):
        ...
        #test convergence order, expected ca. 5
        p, _ = np.polyfit(np.log10(timesteps), np.log10(errors), deg=1)
        self.assertEqual(round(p), 5)
